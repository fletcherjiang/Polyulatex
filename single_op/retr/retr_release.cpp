...
aclError AclFvRelease::PrepareInput(char *args, uint32_t argsSize)
{
    ...
    if (rtRet != RT_ERROR_NONE) {
        ACL_LOG_CALL_ERROR("[Create][Stream]AclFvRelease create stream failed, runtime result = %d.", rtRet);
        return ACL_ERROR_RT_FAILURE;
    }

    aclError ret = aclrtMalloc(&retCode_, sizeof(int32_t), ACL_MEM_MALLOC_NORMAL_ONLY);
    if (ret != ACL_SUCCESS) {
        ACL_LOG_INNER_ERROR("[Malloc][Mem]AclFvRelease alloc device buffer failed, "
...
aclError AclFvRelease::Release()
{
    ...
    rtError_t rtRet = rtCpuKernelLaunch(RETR_KERNELS_SONAME,
        ...
        stream_);
    if (rtRet != RT_ERROR_NONE) {
        ACL_LOG_CALL_ERROR("[Release][Retr]retr release call rtCpuKernelLaunch failed, runtime result = %d.", rtRet);
    ...
    if (rtRet != RT_ERROR_NONE) {
        ACL_LOG_CALL_ERROR("[Sync][Stream]synchronize stream failed, runtime result = %d.", rtRet);
        (void)aclrtFree(retCode_);
        (void)rtStreamDestroy(stream_);
        return ACL_ERROR_RT_FAILURE;
    }
