...
aclError AclFvInit::PrepareInput(aclfvInitPara *initPara, char *args, uint32_t argsSize)
{
    ...
    if (ret != ACL_SUCCESS) {
        ACL_LOG_ERROR("aclrtGetRunMode failed, ret = %d", ret);
        return ret;
    }
    ...
    if (rtRet != RT_ERROR_NONE) {
        ACL_LOG_ERROR("AclFvInit create stream failed, runtime result = %d", rtRet);
...
aclError AclFvInit::Init(aclfvInitPara *initPara)
{
    ...
    rtError_t rtRet = rtCpuKernelLaunch(RETR_KERNELS_SONAME,
        ...
        stream_);
    if (rtRet != RT_ERROR_NONE) {
        ACL_LOG_ERROR("retr initialize call rtCpuKernelLaunch failed, runtime result = %d", rtRet);
        (void)rtStreamDestroy(stream_);
        return ACL_ERROR_RT_FAILURE;
    }
    ...
    if (initPara->initPara.retCode != 0) {
        ACL_LOG_ERROR("aclfvInit failed, ret = %d", initPara->initPara.retCode);
