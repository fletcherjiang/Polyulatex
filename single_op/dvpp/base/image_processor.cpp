...
namespace acl {
    ...
    aclError ImageProcessor::acldvppPngGetImageInfo(const void *data,
                                                    uint32_t dataSize,
                                                    uint32_t *width,
                                                    uint32_t *height,
                                                    int32_t *components)
    {
        ...
        if (ret != ACL_SUCCESS) {
            ACL_LOG_INNER_ERROR("[Get][PngInfo]get png info failed, result = %d.", ret);
            return ret;
        }
    ...
    aclError ImageProcessor::acldvppPngPredictDecSize(const void *data,
                                                      uint32_t dataSize,
                                                   ...
                                                   aclrtStream stream)
    {
        ...
        *sizeVal = size;
        ...
        if (aclRunMode_ == ACL_HOST) {
            aclError cpyRet = CopyDvppPicDescAsync(outputDesc, ACL_MEMCPY_DEVICE_TO_HOST, stream);
            if (cpyRet != ACL_SUCCESS) {
                ACL_LOG_INNER_ERROR("[Copy][Desc]copy output pic desc from device failed, result = %d.", cpyRet);
                return cpyRet;
            }
        }
    ...
    aclError ImageProcessor::GetPngImgInfo(const void *data,
                                               uint32_t dataSize,
                                               uint32_t *width,
                                               uint32_t *height,
                                               int32_t *components,
