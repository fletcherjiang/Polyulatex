...
static aclError GeFinalizeFunc()
{
    ...
    if (geRet != SUCCESS) {
        ACL_LOG_ERROR("ge finalize failed. ge result = %u", geRet);
...
aclError LocalCompiler::Init(const std::map<std::string, std::string> &options)
{
    ...
    if (isInitialized_) {
        ACL_LOG_ERROR("Compiler already initialized");
    ...
    if (geRet != SUCCESS) {
        ACL_LOG_ERROR("GEInitialize failed. ge result = %u", geRet);
        return ACL_GET_ERRCODE_GE(geRet);
    }
...
aclError LocalCompiler::OnlineCompile(CompileParam &param, std::shared_ptr<void> &modelData, size_t &modelSize)
{
    ...
    if (geRet != SUCCESS) {
        ACL_LOG_ERROR("call ge interface generator.Initialize failed. ge result = %u", geRet);
        return ACL_GET_ERRCODE_GE(geRet);
    }
    ...
    if (geRet != SUCCESS) {
        ACL_LOG_ERROR("call ge interface generator.BuildSingleOpModel failed. ge result = %u", geRet);
        return ACL_GET_ERRCODE_GE(geRet);
    }

    geRet = generator.Finalize();
    if (geRet != SUCCESS) {
        ACL_LOG_ERROR("call ge interface generator.Finalize failed. ge result = %u", geRet);
        return ACL_GET_ERRCODE_GE(geRet);
    }
