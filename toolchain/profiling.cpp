...
namespace acl {
    aclError AclProfiling::HandleProfilingCommand(const std::string &config, bool configFileFlag)
    {
        ...
        if (configFileFlag) {
            ...
            if (ret != 0) {
                ACL_LOG_ERROR("handle json config of profiling failed, profiling result = %d", ret);
                return ACL_ERROR_INVALID_PARAM;
            }
        } else {
            int32_t ret = callback(MSPROF_CTRL_INIT_ACL_ENV, const_cast<char *>(config.c_str()), config.size());
            if (ret != 0) {
                ACL_LOG_ERROR("handle env config of profiling failed, profiling result = %d", ret);
                return ACL_ERROR_INVALID_PARAM;
            }
        }
    ...
    aclError AclProfiling::HandleProfilingConfig(const char *configPath)
    {
        ...
        if (ret != ACL_SUCCESS) {
            ACL_LOG_ERROR("handle profiling command failed, result = %d", ret);
            return ret;
        }
