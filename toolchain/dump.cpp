...
namespace acl {
    ...
    static std::vector<std::string> Split(const std::string &str, const char *delimiter)
    {
        ...
        if (str.empty()) {
            ACL_LOG_ERROR("[Check][Str]input str is null");
            return resVec;
        }

        if (delimiter == nullptr) {
            ACL_LOG_ERROR("[Check][delimiter]delimiter is nullptr");
            return resVec;
        }
    ...
    static bool CheckDumplist(const nlohmann::json &js)
    {
        ...
        if (dumpOpSwitch == ACL_DUMP_OP_SWITCH_OFF) {
            ...
            if (!isValidDumpList) {
                ACL_LOG_ERROR("[Check][ValidDumpList]dump_list is not null, but dump_list "
    ...
    static bool IsValidDumpConfig(const nlohmann::json &js)
    {
        ...
        if (!CheckDumplist(js)) {
            ACL_LOG_ERROR("[Check][Dumplist]dump_list or dump_op_switch field is invalid");
            return false;
        }
    ...
    aclError ConvertDumpConfig(const nlohmann::json &js, ge::DumpConfig &dumpConfig)
    {
        ...
        if (!IsValidDumpConfig(js)) {
            ACL_LOG_ERROR("[Check][DumpConfig]dump config is invalid");
        ...
        for (size_t i = 0; i < config.dumpList.size(); ++i) {
            if (config.dumpList[i].modelName.empty()) {
                ACL_LOG_ERROR("[Check][modelName]the %zu modelName field is null", i);
                continue;
            }
            if (config.dumpList[i].isLayer && (config.dumpList[i].layer.empty())) {
                ACL_LOG_ERROR("[Check][Layer]layer field is null in model %s",
    ...
    aclError AclDump::HandleDumpCommand(ge::DumpConfig &dumpConfig)
    {
        ...
        if (adxRet != ADX_ERROR_NONE) {
            ACL_LOG_ERROR("[AdxDataDumpServer][Init]dump server run failed, adx result = %d", adxRet);
        ...
        if (geRet != ge::SUCCESS) {
            ACL_LOG_ERROR("[Set][Dump]set dump config for model failed, ge result = %d", geRet);
    ...
    aclError AclDump::HandleDumpConfig(const char *configPath)
    {
        ...
        if (ret != ACL_SUCCESS) {
            ACL_LOG_ERROR("[Parse][JsonFromFile]parse dump config from file[%s] failed, result = %d",
                configPath, ret);
            return ret;
        }
        try {
            if (js.find(ACL_DUMP) != js.end()) {
                ...
                if (ret != ACL_SUCCESS) {
                    ACL_LOG_ERROR("[Convert][DumpConfig]convert to ge dump config file failed, "
                        "result = %d", ret);
                    return ACL_SUCCESS;
                }
            ...
            ACL_LOG_ERROR("[Convert][DumpConfig]parse json for config failed, exception:%s.",
                e.what());
            return ACL_ERROR_INVALID_DUMP_CONFIG;
        }
...
aclError aclmdlInitDump()
{
    ...
    if (!GetAclInitFlag()) {
        ACL_LOG_ERROR("[Check][AclInitFlag]aclmdlInitDump is not support because it does not execute aclInit");
        return ACL_ERROR_UNINITIALIZE;
    }

    if (acl::AclDump::GetInstance().GetAclDumpFlag()) {
        ACL_LOG_ERROR("[Check][AclDumpFlag]aclmdlInitDump is not support because already execute aclInit init dump");
        return ACL_ERROR_DUMP_ALREADY_RUN;
    }

    std::unique_lock<std::mutex> lk(aclDumpMutex);
    if (aclmdlInitDumpFlag) {
        ACL_LOG_ERROR("[Check][InitDumpFlag]repeatedly initialized dump in aclmdlInitDump, "
...
aclError aclmdlSetDump(const char *dumpCfgPath)
{
    ...
    ACL_STAGES_REG(acl::ACL_STAGE_DUMP, acl::ACL_STAGE_DEFAULT);
    if (!GetAclInitFlag()) {
        ACL_LOG_ERROR("[Check][AclInitFlag]aclmdlSetDump is not support because it does not execute aclInit");
        return ACL_ERROR_UNINITIALIZE;
    }

    if (acl::AclDump::GetInstance().GetAclDumpFlag()) {
        ACL_LOG_ERROR("[Check][AclDumpFlag]aclmdlSetDump is not support because already execute aclInit");
        return ACL_ERROR_DUMP_ALREADY_RUN;
    }

    std::unique_lock<std::mutex> lk(aclDumpMutex);
    if (!aclmdlInitDumpFlag) {
        ACL_LOG_ERROR("[Check][aclmdlInitDumpFlag]dump is not initialized in aclmdlInitDump");
    ...
    try {
        ret = acl::ConvertDumpConfig(js, dumpConfig);
        if (ret != ACL_SUCCESS) {
            ACL_LOG_ERROR("[Convert][DumpConfig]convert to ge dump config file failed, result = %d", ret);
            return ACL_ERROR_INVALID_DUMP_CONFIG;
        }
    } catch (const nlohmann::json::exception &e) {
        ACL_LOG_ERROR("[Convert][DumpConfig]invalid dump config, exception:%s", e.what());
        return ACL_ERROR_INVALID_DUMP_CONFIG;
    }

    ge::Status geRet = geExecutor.SetDump(dumpConfig);
    if (geRet != ge::SUCCESS) {
        ACL_LOG_ERROR("[Set][DumpConfig]set dump config for model failed, ge result = %d", geRet);
        return ACL_GET_ERRCODE_GE(geRet);
    }
...
aclError aclmdlFinalizeDump()
{
    ...
    ACL_STAGES_REG(acl::ACL_STAGE_DUMP, acl::ACL_STAGE_DEFAULT);
    if (!GetAclInitFlag()) {
        ACL_LOG_ERROR("[Check][AclInitFlag]aclmdlFinalizeDump is not support because it does not execute aclInit");
        return ACL_ERROR_UNINITIALIZE;
    }

    if (acl::AclDump::GetInstance().GetAclDumpFlag()) {
        ACL_LOG_ERROR("[Check][AclDumpFlag]aclmdlFinalizeDump is not support because already execute aclInit");
        return ACL_ERROR_DUMP_ALREADY_RUN;
    }
    ...
    if (aclmdlInitDumpFlag) {
        ...
        if (adxRet != ADX_ERROR_NONE) {
            ACL_LOG_ERROR("[AdxDataDumpServer][UnInit]generate dump file failed in disk, adx result = %d", adxRet);
            return ACL_ERROR_INTERNAL_ERROR;
        }
    } else {
        ACL_LOG_ERROR("[Check][aclmdlInitDumpFlag]dump is not initialized in aclmdlInitDump");
