...
bool AttrValueEquals(const ge::GeAttrValue &lhs, const ge::GeAttrValue &rhs)
{
    ...
    switch (type) {
        ...
        default:
            ACL_LOG_ERROR("Unknown type %d", static_cast<int32_t>(type));
            return false;
    }
}
    ...
    for (int i = 0; i < tensorNum; ++i) {
        ...
        if (desc->isConst) {
            if ((desc->constDataBuf != nullptr) && (desc->constDataLen <= 0)) {
                ACL_LOG_ERROR("constDataBuf is not nullptr and dataLen is <= 0");
                return false;
            }
            if ((desc->constDataBuf == nullptr) && (desc->constDataLen > 0)) {
                ACL_LOG_ERROR("constDataBuf is nullptr and dataLen is > 0");
                return false;
            }
...
bool SaveConstToAttr(const AclOp &aclOp, aclopAttr *opAttr)
{
    ...
    if (!ret) {
        ACL_LOG_ERROR("inputTenspr get const dataLen failed");
        return false;
    }
    ret = ConstToAttr(aclOp.numOutputs, aclOp.outputDesc, constStr);
    if (!ret) {
        ACL_LOG_ERROR("outputTensor get const dataLen failed");
        return false;
    }
