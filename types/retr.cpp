...
void aclfvDestoryType(Type type)
{
    if (type != nullptr) {
        if (type->dataBuffer.data != nullptr) {
            ...
            if (ret != ACL_SUCCESS) {
                ACL_LOG_ERROR("[Free][Mem]free device mem failed, result = %d.", ret);
            }
...
aclfvInitPara *aclfvCreateInitPara(uint64_t fsNum)
{
    ...
    if (initPara == nullptr) {
        ACL_LOG_ERROR("[Check][InitPara]create acl fv init param failed.");
        return nullptr;
    }
    ...
    if (ret != ACL_SUCCESS) {
        ACL_LOG_ERROR("[Malloc][Mem]malloc device memory for acl fv init param failed, result = %d.", ret);
    ...
    uint32_t featureCount, uint8_t *featureData, uint32_t featureDataLen)
{
    ...
    if (featureCount > RETR_FEATURE_COUNT_MAX) {
        ACL_LOG_ERROR("[Check][FeatureCount]the value of featureCount[%u] can't be larger than "
            "RETR_FEATURE_COUNT_MAX[%u].", featureCount, RETR_FEATURE_COUNT_MAX);
        return nullptr;
    }

    if (featureDataLen != featureLen * featureCount) {
        ACL_LOG_ERROR("[Check][FeatureDataLen]featureDataLen:%u should be equal to featureLen:%u * featureCount:%u.",
            featureDataLen,
            featureLen,
            featureCount);
        return nullptr;
    ...
    if (aclFeatureInfo == nullptr) {
        ACL_LOG_ERROR("[Check][FeatureInfo]create acl retr feature info failed.");
        return nullptr;
    }
    ...
    aclError ret = aclrtMalloc(&devPtr, size, ACL_MEM_MALLOC_NORMAL_ONLY);
    if (ret != ACL_SUCCESS) {
        ACL_LOG_ERROR("[Check][Mem]malloc device memory for acl retr feature info, result = %d.", ret);
...
aclfvRepoRange *aclfvCreateRepoRange(uint32_t id0Min, uint32_t id0Max, uint32_t id1Min, uint32_t id1Max)
{
    ...
    if (aclRepoRange == nullptr) {
        ACL_LOG_ERROR("[Check][RepoRange]create acl retr RepoRange failed.");
        return nullptr;
    }
    ...
    aclError ret = aclrtMalloc(&devPtr, size, ACL_MEM_MALLOC_NORMAL_ONLY);
    if (ret != ACL_SUCCESS) {
        ACL_LOG_ERROR("[Malloc][Mem]malloc device memory for acl retr RepoRange, result = %d.", ret);
...
aclfvQueryTable *aclfvCreateQueryTable(uint32_t queryCnt, uint32_t tableLen, uint8_t *tableData, uint32_t tableDataLen)
{
    ...
    if (retrQueryTable == nullptr) {
        ACL_LOG_ERROR("[Check][RetrQueryTable]create acl retr query table failed.");
        return nullptr;
    }
...
aclfvSearchInput *aclfvCreateSearchInput(aclfvQueryTable *queryTable, aclfvRepoRange *repoRange, uint32_t topK)
{
    ...
    if (repoRange == nullptr) {
        ...
        REPORT_INPUT_ERROR(acl::INVALID_NULL_POINTER_MSG, std::vector<std::string>({"param"}),
            std::vector<std::string>({"repoRange"}));
        return nullptr;
    }
    ...
    if (retrSearchInput == nullptr) {
        ACL_LOG_ERROR("[Check][RetrSearchInput]create acl retr search input failed.");
        return nullptr;
    }
    ...
    aclError ret = aclrtMalloc(&devPtr, size, ACL_MEM_MALLOC_NORMAL_ONLY);
    if (ret != ACL_SUCCESS) {
        ACL_LOG_ERROR("[Malloc][Mem]malloc device memory for acl retr search input failed, "
            "result = %d.", ret);
    ...
    uint32_t *id0, uint32_t *id1, uint32_t *resultOffset, float *resultDistance, uint32_t dataLen)
{
    if (resultNum == nullptr) {
        ACL_LOG_ERROR("[Check][Mem]create acl retr search result failed, resultNum is nullptr.");
    ...
    if (id0 == nullptr) {
        ACL_LOG_ERROR("[Check][id0]create acl retr search result failed, id0 is nullptr.");
        REPORT_INPUT_ERROR(acl::INVALID_NULL_POINTER_MSG,
            std::vector<std::string>({"param"}), std::vector<std::string>({"id0"}));
        return nullptr;
    }

    if (id1 == nullptr) {
        ACL_LOG_ERROR("[Check][id1]create acl retr search result failed, id1 is nullptr.");
        REPORT_INPUT_ERROR(acl::INVALID_NULL_POINTER_MSG,
    ...
    if (resultOffset == nullptr) {
        ACL_LOG_ERROR("[Check][resultOffset]create acl retr search result failed, resultOffset is nullptr.");
        REPORT_INPUT_ERROR(acl::INVALID_NULL_POINTER_MSG,
            std::vector<std::string>({"param"}),
    ...
    if (resultDistance == nullptr) {
        ACL_LOG_ERROR("[Check][resultDistance]create acl retr search result failed, resultDistance is nullptr.");
        REPORT_INPUT_ERROR(acl::INVALID_NULL_POINTER_MSG,
            std::vector<std::string>({"param"}),
    ...
    if (retrSearchResult == nullptr) {
        ACL_LOG_ERROR("[Check][RetrSearchResult]create acl retr search result failed.");
        return nullptr;
    }
    ...
    aclError ret = aclrtMalloc(&devPtr, size, ACL_MEM_MALLOC_NORMAL_ONLY);
    if (ret != ACL_SUCCESS) {
        ACL_LOG_ERROR("[Check][DevPtr]malloc device memory for acl retr search input failed, result = %d.", ret);
...
aclError aclfvDestroySearchInput(aclfvSearchInput *retrSearchInput)
{
    if (retrSearchInput != nullptr) {
        if (retrSearchInput->dataBuffer.data != nullptr) {
            ...
            if (ret != ACL_SUCCESS) {
                ACL_LOG_ERROR("[Check][Mem]free device mem failed, result = %d.", ret);
            }
